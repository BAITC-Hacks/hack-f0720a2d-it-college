...
def _client(connection: Connection, *, discovery: bool = False) -> httpx.Client:
    if not connection.api_key.isascii() or any(ord(char) < 32 or ord(char) == 127 for char in connection.api_key):
        raise HTTPException(503, "Проверьте формат API-ключа совместимого сервера в настройках AI.")
    return httpx.Client(
...
def _request(client: httpx.Client, method: str, path: str, **kwargs) -> httpx.Response:
    try:
        ...
        raise HTTPException(504, "AI-сервер не ответил вовремя. Проверьте загрузку модели и повторите запрос.") from None
    except httpx.RequestError:
        raise HTTPException(503, "Нет связи с AI-сервером. Запустите сервер LM Studio или проверьте адрес в настройках AI.") from None
        ...
        for attempt in range(2):
            ...
            try:
                ...
                if attempt == 0:
                    ...
                    continue
                raise HTTPException(502, "AI дважды вернул некорректный ответ или сведения без источника. Карточка не изменена. Повторите запрос или выберите другую модель.") from None
