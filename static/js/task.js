...
export async function renderTask(root, ctx, taskId) {
  ...
  root.innerHTML = '<div class="stack"><nav class="breadcrumbs"><a href="#catalog">Каталог</a><span>/</span><span>' + esc(titleOf(task)) + '</span></nav>' +
    (task.has_pending_changes ? alertBox("info", "Предварительная версия", "Изменения видны только вам. Проверьте их в редакторе и подтвердите, чтобы обновить каталог.") + linkButton("Подтвердить изменения", "#publish/" + task.id, "secondary") : '') +
