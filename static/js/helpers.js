...
export function ratingPanel(task, { actions = "", hints = true } = {}) {
  ...
  return '<section class="panel rating-panel"><div class="eyebrow">Рейтинг готовности</div><div class="rating-total"><span class="score score--lg">' + fmt(task.score) + '</span><span class="caption">из 100</span>' + badge(task.score) + "</div>" + meter(task.score) + '<div class="rating-ticks mono"><span>0</span><span>40</span><span>70</span><span>90</span><span>100</span></div>' + preview + '<div class="breakdown">' + breakdown(task.indicators) + "</div>" + (hints && task.missing?.length ? '<div><div class="eyebrow">Что повысит рейтинг</div>' + missingList(task.missing) + "</div>" : "") + '<details class="rating-details"><summary>Как считается рейтинг</summary><p class="caption">Баллы только за заполненные и вручную подтверждённые поля. Пустое или неподтверждённое поле, заглушка или повтор — 0; меньше 30 символов — половина веса; от 30 — полный вес. Контекст и потребность: по 10 баллов. Правка снимает подтверждение изменённого поля. Отклик открыт при любом уровне.</p></details>' + actions + "</section>";
}
